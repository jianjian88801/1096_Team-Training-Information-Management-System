    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="qiuduiId" name="qiuduiId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="upload" v-if="type!='info' && !ro.shoufamingdanFile" label="名单下载" prop="shoufamingdanFile">
                        <file-upload
                                tip="点击上传名单下载"
                                action="file/upload"
                                :limit="3"
                                :multiple="true"
                                :fileUrls="ruleForm.shoufamingdanFile?ruleForm.shoufamingdanFile:''"
                                @change="shoufamingdanFileUploadChange"
                        ></file-upload>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.shoufamingdanFile" label="名单下载" prop="shoufamingdanFile">
                            <a type="text" style="text-decoration:none" class="el-button" :href="ruleForm.shoufamingdanFile"  >名单下载下载</a>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
        ...
        data() {
            return {
                ...
                rules: {
                   ...
                   shoufamingdanFile: [
                              { required: true, message: '名单下载不能为空', trigger: 'blur' },
                          ],
                }
            };
        },
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`shoufamingdan/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.shoufamingdanCrossAddOrUpdateFlag = false;
                                        this.parent.search();
            ...
            //图片
            shoufamingdanFileUploadChange(fileUrls){
                this.ruleForm.shoufamingdanFile = fileUrls;
                this.addEditUploadStyleChange()
            },
